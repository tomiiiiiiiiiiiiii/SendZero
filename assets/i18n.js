(() => {
  ...
  const messages = {
    en: {
      ...
      privacy_4_text: 'LocalStorage is used for language choice and resumable-upload state, which can include an upload token and the encryption key needed to resume that upload. IndexedDB may store resumable-download state such as a local file handle, verified byte offset, key fingerprint and download session token. Clearing browser storage removes this local state.',
    ...
    de: {
      ...
      privacy_4_text: 'LocalStorage wird für die Sprachauswahl und den Zustand fortsetzbarer Uploads genutzt; dieser kann ein Upload-Token und den für die Fortsetzung benötigten Verschlüsselungsschlüssel enthalten. IndexedDB kann Daten für fortsetzbare Downloads speichern, etwa lokalen Datei-Handle, bestätigten Byte-Offset, Schlüssel-Fingerabdruck und Download-Session-Token. Das Löschen des Browser-Speichers entfernt diesen lokalen Zustand.',
